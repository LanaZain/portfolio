...
const fetchAboutMe = async () => {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Error fetching aboutMe: ${response.status} ${response.statusText}`
            )
        }
...
const updateSpotlight = (project) => {
    ...
    const spotlightImage = safeValue(
...
const handleCardClick = (event) => {
    ...
    const project = projectsData.find(
        (p) => String(p.project_id) === String(card.id)
    )
...
const validateMessage = (message) => {
    ...
    if (illegalCharsRegex.test(message)) {
        return 'Message contains illegal characters.'
    }

    if (message.length > 300) {
        return `Message is too long (${message.length}/300 characters).`
    }
...
const handleFormSubmit = (event) => {
    ...
    if (!emailError && !messageError) {
        alert(
            'Validation successful! Your message has been submitted.'
        )
...
const updateCharCount = (count) => {
    const charCount =
        document.querySelector('#charactersLeft')
